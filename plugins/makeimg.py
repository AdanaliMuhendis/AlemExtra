...
async def draw(_: app, m: Message):
    global Database
    if len(m.command) == 1:
        return await m.reply_text("ᴘʟᴇᴀsᴇ ᴘʀᴏᴠɪᴅᴇ ᴀ ᴘʀᴏᴍᴘᴛ. ᴜsᴀɢᴇ: `/draw <prompt>`", parse_mode=enums.ParseMode.MARKDOWN)
    prompt = getText(m)
    user = m.from_user
...
async def selectModel(_:app,query:CallbackQuery):
    ...
    images[-1] = InputMediaPhoto(img_url[-1],caption=f"Your Prompt:\n`{promptData['prompt']}`")
